...
function VillageJediManager:canSurrenderSkill(pPlayer, skillName)
	...
	if string.find(skillName, "jedi") and CreatureObject(pPlayer):hasSkill("force_title_jedi_rank_03") and not CreatureObject(pPlayer):villageKnightPrereqsMet(skillName) then
		return false
	end
